...
class RESTasV3:
    ...
    def __sendPost(self, url, payload, customHeaders=None, files=None, debug=True):
        ...
        if DEBUG:print("POST at URL: {} with payload: {}".format(url, payload))
        ...
        if debug:
            if DEBUG:print("POST response message: {}, response code: {}".format(response.content, response.status_code))
        if response.status_code == 401:
            if DEBUG:print('Token has expired, resending request')
    ...
    def __sendGet(self, url, expectedResponse, customHeaders=None, debug=True):
        if DEBUG:print("GET at URL: {}".format(url))
        response = self.session.get('{}{}'.format(self.host, url),
                                    headers=customHeaders if customHeaders else self.headers)
        if debug:
            if DEBUG:print("GET response message: {}, response code: {}".format(response.content, response.status_code))
        if response.status_code == 401:
            if DEBUG:print('Token has expired, resending request')
            self.refresh_access_token()
            response = self.session.get('{}{}'.format(self.host, url),
    ...
    def __sendPut(self, url, payload, customHeaders=None, debug=True):
        if DEBUG:print("PUT at URL: {} with payload: {}".format(url, payload))
        ...
        if debug:
            if DEBUG:print("PUT response message: {}, response code: {}".format(response.content, response.status_code))
        if response.status_code == 401:
            if DEBUG:print('Token has expired, resending request')
            self.refresh_access_token()
            response = self.session.put('{}{}'.format(self.host, url),
    ...
    def __sendPatch(self, url, payload, debug=True):
        if DEBUG:print("PATCH at URL: {} with payload: {}".format(url, payload))
        expectedResponse = [200, 204]
        response = self.session.patch('{}{}'.format(self.host, url), headers=self.headers, data=json.dumps(payload))
        if debug:
            if DEBUG:print("PATCH response message: {}, response code: {}".format(response.content, response.status_code))
        if response.status_code == 401:
            if DEBUG:print('Token has expired, resending request')
            self.refresh_access_token()
            response = self.session.patch('{}{}'.format(self.host, url), headers=self.headers, data=json.dumps(payload))
    ...
    def __sendDelete(self, url, headers=None, debug=True):
        if DEBUG:print("DELETE at URL: {}".format(url))
        expectedResponse = [200, 202, 204]
        response = self.session.delete('%s%s' % (self.host, url), headers=headers)
        if debug:
            if DEBUG:print("DELETE response message: {}, response code: {}".format(response.content, response.status_code))
        if response.status_code == 401:
            if DEBUG:print('Token has expired, resending request')
            self.refresh_access_token()
            response = self.session.delete('%s%s' % (self.host, url), headers=headers)
    ...
    def get_automation_token(self):
        ...
        if self.verify:
            if response.headers.get('content-type') == 'application/json':
                ...
                if DEBUG:print('Access Token: {}'.format(response["access_token"]))
    ...
    def refresh_access_token(self):
        ...
        if DEBUG:print('Authentication token refreshed!')
    ...
    def deactivate_license(self, activation_code, quantity=1, timeout=60):
        ...
        if "The Activation Code : \'{}\' is not installed.".format(activation_code) == \
            ...
            if DEBUG:print('License code {} is not installed'.format(activation_code))
    ...
    def import_config(self, config):
        ...
        if response:
            if DEBUG:print('Config successfully imported, config ID: {}'.format(response.json()[0]['id']))
    ...
    def export_config(self, export_path=None):
        ...
        if DEBUG:print("Export path/file: {}".format(file_name))
    ...
    def open_config(self):
        ...
        if response:
            if DEBUG:print('Config successfully opened, session ID: {}'.format(response.json()[0]['id']))
            return response.json()[0]['id']
    ...
    def check_agents_status(self, timeout=60):
        waiting_time = 0
        if DEBUG:print('Waiting for agents to be available')
        while True:
    ...
    def get_agents_ids(self, agentIPs=None, wait=None):
        ...
        agentsIDs = list()
        response = self.get_agents()
        if DEBUG:print('Found {} agents'.format(len(response)))
        if type(agentIPs) is str:
            agentIPs = [agentIPs]
        for agentIP in agentIPs:
            for agent in response:
                if agent['IP'] == agentIP:
                    if DEBUG:print("agent_IP: {}, agent_ID: {}".format(agent['IP'], agent['id']))
    ...
    def get_agents_ips(self, wait=None):
        ...
        if DEBUG:print('Agents IP List: {}'.format(agentsIPs))
        return agentsIPs
    ...
    def get_iteration_count_info(self, ap_id=1):
        ...
        if config_type["traffic"]:
            if DEBUG:print('Parameter not available in traffic profile')
    ...
    def stop_test(self, stopTimeout=60):
        ...
        response = self.__sendPost(apiPath, payload={}).json()
        stopID = response['id']
        if DEBUG:print('Stop ID : {}'.format(stopID))
        ...
        while iteration < stopTimeout:
            ...
            if DEBUG:print('Stop Test Progress: {}'.format(response['progress']))
            if response['state'] == 'SUCCESS':
                break
    ...
    def abort_test(self, abortTimeout=60):
        ...
        response = self.__sendPost(apiPath, payload={}).json()
        stopID = response['id']
        if DEBUG:print('Abort ID : {}'.format(stopID))
        ...
        while iteration < abortTimeout:
            response = self.__sendGet(progressPath, 200).json()
            if DEBUG:print('Abort Test Progress: {}'.format(response['progress']))
            if response['state'] == 'SUCCESS':
                break
            if response['state'] == 'ERROR':
    ...
    def wait_test_finished(self, timeout=300):
        if DEBUG:print('Waiting for the test to finish...')
        response = self.get_test_status()
        actual_duration = 0
        counter = 1
        while actual_duration < self.testDuration + timeout:
            ...
            if response['status'] == 'STOPPED':
                if response['testElapsed'] >= response['testDuration']:
                    if DEBUG:print('Test gracefully finished')
            ...
            if DEBUG:print("Test did not stop after timeout {}s. Test status= {}. Force stopping the test!".format(timeout,
                                                                                                          response[
                                                                                                              'status']))
            self.stop_test()
    ...
    def get_stats_values(self, statName):
        if DEBUG:print('Get the values for {}'.format(statName))
    ...
    def get_application_id(self, app_name):
        ...
        if DEBUG:print('Getting application {} ID...'.format(app_name))
        for app in self.app_list:
            if app['Name'] == app_name:
                if DEBUG:print('Application ID = {}'.format(app['id']))
                return app['id']
    ...
    def get_strike_id(self, strike_name):
        ...
        for strike in self.strike_list:
            if strike['Name'] == strike_name:
                if DEBUG:print('Strike ID = {}'.format(strike['id']))
                return strike['id']
    ...
    def set_custom_payload(self, apiPath, fileName):
        ...
        if resp.status_code != 204:
            if DEBUG:print("Error patching payload type: {}".format(resp.json()))
    ...
    def get_config_id(self, test_name):
        ...
        for config in configs:
            if config['displayName'] == test_name:
                if DEBUG:print('Config ID = {}'.format(config['id']))
                return config['id']
    ...
    def load_config(self, test_name):
        ...
        if response:
            if DEBUG:print('Test= {} was loaded with ID= {}'.format(test_name, response[-1]['id']))
    ...
    def set_tunnel_count(self, number_of_tunnels, network_segment_number=1):
        ...
        if resp.status_code != 204:
            if DEBUG:print("Error setting VPN tunnel count per outer IP: {}".format(resp.json()))
    ...
    def set_tunnel_outer_ip_gateway(self, gateway_ip, network_segment_number=1):
        ...
        if resp.status_code != 204:
            if DEBUG:print("Error network tags in Inncer IP range: {}".format(resp.json()))
    ...
    def import_controller(self, import_path=None, file_name="fakenews.zip"):
        ...
        if DEBUG:print(f"This is post {result}")
